    ...
    <div class="row">
      ...
      <div class="col-md-6 sdc-user-right">
        ...
        <div class="user-right-bot">
          <p>转出/入记录列表</p>
          <table>
            <tr align="center" class="volume-table-header">
              <td>时间</td>
              <td>SDC数量</td>
              <td>转至账户</td>
              <td>转出账户</td>
            </tr>
            <?php if(is_array($list)): foreach($list as $key=>$var): ?><tr class="volume-table-list" align="center">
              <td><?php echo date('Y/m/d',$var['add_time']);?></td>
              <td><?php if($var['to'] == $selfEmail): ?><span class="font_red">+</span><?php else: ?><span class="font_red">-</span><?php endif; echo ($var['amount']); ?></td>
              <td><?php if($var['to'] == $selfEmail): ?><span>自己</span><?php else: ?><span><?php echo ($var['to']); ?></span><?php endif; ?></td>
              <td><?php if($var['from'] == $selfEmail): ?><span>自己</span><?php else: ?><span><?php echo ($var['from']); ?></span><?php endif; ?></td>
